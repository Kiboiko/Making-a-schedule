...
namespace Shedule
{
    public class mainMethod
    {
        ...
        //----------------------------------------------------
        public static List<List<Teacher>> GetTeacherComboForTheTimeSlot(List<Student> students, List<Teacher> teachers)
        {
            List<List<Teacher>> uniqTeachers = HelperMethods.GetAllTeacherCombinations(teachers).OrderBy(x => x.Count).ToList();
            List<List<Teacher>> res = new List<List<Teacher>>();
            foreach (var combo in uniqTeachers)
            {
                if (CheckTeachersComboForTheTimeSlot(students, combo) && CheckForEntryinterruption(res, combo))
                {
                    res.Add(combo);
                }
            }
        ...
        public static bool CheckTeachersComboForTheTimeSlot(List<Student> students, List<Teacher> teachers)
        {
            TimeOnly startStudTime = students.Select(x => x.StartOfStudyingTime).ToList().Min();
            TimeOnly currentTime = TimeOnly.FromTimeSpan(TimeSpan.FromMinutes(15));
            bool t = true;
            for (int i = 0; i < 15; i++)
            {
                currentTime = currentTime.AddMinutes(15);
        ...
        public static object[,] GenerateTeacherScheduleMatrix(List<Student> students, List<Teacher> teachers)
        {
            ...
            for (int slot = 1; slot <= timeSlots; slot++)
            {
                ...
                List<Teacher> activeTeachers = teachers
                    .Where(s => s.StartOfStudyingTime < slotEnd &&
                               s.EndOfStudyingTime >= slotStart)
                    .ToList();

                List<List<Teacher>> teachCombo = new List<List<Teacher>>();
                var combos = HelperMethods.GetAllTeacherCombinations(activeTeachers);
